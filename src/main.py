...
app = FastAPI(title="SSEI API", version="0.2.0")

# CORS — permite conexiones desde la app móvil (dev) y el panel admin (LAN)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],   # En producción, limitar a IPs/dominios de la LAN
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def _ot_to_read(ot) -> OtRead:
    return OtRead(
        ...
        atms=[
            {
                "id": a.id, "ot_id": a.ot_id, "etiqueta": a.etiqueta,
                "tipo_servicio": a.tipo_servicio, "numero_atm": a.numero_atm,
                "serie_cajero": a.serie_cajero, "serie_mmbb": a.serie_mmbb,
                "detalles_servicio": a.detalles_servicio, "observaciones": a.observaciones,
            }
            for a in ot.atms
        ],
    )
...
def list_ots(estado: str | None = Query(default=None)) -> list[OtRead]:
    """Lista todas las OTs. Filtrar por ?estado=asignado|en_progreso|pendiente_envio|sincronizado"""
...
def create_ot(payload: OtCreate) -> OtRead:
    """Crear una nueva OT (coordinador asigna trabajo)."""
...
def get_ot(ot_id: int) -> OtRead:
    """Obtener detalle completo de una OT con sus ATMs."""
...
def update_ot(ot_id: int, payload: OtUpdate) -> OtRead:
    """Actualizar datos de una OT y reemplazar sus ATMs."""
...
def update_ot_estado(ot_id: int, payload: OtEstadoUpdate) -> OtRead:
    """Cambiar el estado de una OT: asignado → en_progreso → pendiente_envio → sincronizado."""
...
def delete_ot(ot_id: int) -> None:
    """Eliminar una OT y todos sus ATMs asociados."""
